...
def trim_to_half_with_gt(line):
    ...
    if len(words) <= 4 and words[0] == "<s>" and words[-1] == "</s>":
...
input_dir = "../../../CodeCompletion-token/dataset/javaCorpus/token_completion/"
output_dir = "0.01/20/"
# classifier: PREDICTION_DATA_FOLDER_PATH=../../CodeCompletion-token/dataset/javaCorpus/${Percentage}/${SAMPLE_RATIO}/

java_code_cut(input_dir + "train.txt", output_dir + "train_victim.json")
java_code_cut(input_dir + "test.txt", output_dir + "test_victim.json")
# train_20.txt generated by pipline_java.sh
# SAMPLE_RATIO=20 -> num_lines=2586
java_code_cut(input_dir + "train_20.txt", output_dir + "train_surrogate.json", num_lines=2372) # not sure
java_code_cut(input_dir + "test.txt", output_dir + "test_surrogate.json", num_lines=2372)
